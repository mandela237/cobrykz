...
**Architecture:** Keep the implementation isolated to `MobileHero.tsx`. Render one centered, unfiltered `object-cover` video as the absolute backdrop, place a localized semi-transparent gradient inside the text block only, and anchor the content group at bottom-left; enforce the structural contract with the existing source-level presentation test.
